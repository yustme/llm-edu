...
/** Options for fullscreen stepper: null (collapsed) + each capability type */
const EXPAND_OPTIONS: (CapabilityType | null)[] = [
  null,
...
export function Step5Capabilities() {
  ...
  const expandIndex = EXPAND_OPTIONS.indexOf(expandedType);
  const setExpandByIndex = useCallback(
    (i: number) => setExpandedType(EXPAND_OPTIONS[i]),
    [],
  );
  useFullscreenStepper(expandIndex, EXPAND_OPTIONS.length, setExpandByIndex);
