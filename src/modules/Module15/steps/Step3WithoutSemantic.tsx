...
/** Total positions: 0 (empty) + 3 runs + 1 (spread summary) = 5 */
const TOTAL_POSITIONS = INCONSISTENT_RUNS.length + 2;

export function Step3WithoutSemantic() {
  const [visibleRuns, setVisibleRuns] = useState(0);

  const setPosition = useCallback((pos: number) => setVisibleRuns(pos), []);
  useFullscreenStepper(visibleRuns, TOTAL_POSITIONS, setPosition);
