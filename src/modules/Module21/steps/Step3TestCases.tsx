...
export function Step3TestCases() {
  ...
  const typeIndex = TEST_TYPES.indexOf(activeType);
  const setTypeByIndex = useCallback(
    (i: number) => setActiveType(TEST_TYPES[i]),
    [],
  );
  useFullscreenStepper(typeIndex, TEST_TYPES.length, setTypeByIndex);
